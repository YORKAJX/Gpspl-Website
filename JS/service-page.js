document.addEventListener("DOMContentLoaded", () => {
    ...
    function setSeo(data) {
        ...
        document.querySelector('meta[name="description"]')?.setAttribute("content", data.seo.metaDescription);
        document.querySelector('meta[name="keywords"]')?.setAttribute("content", data.seo.keywords);
        document.querySelector('meta[name="robots"]')?.setAttribute("content", data.seo.indexing);
        document.querySelector('link[rel="canonical"]')?.setAttribute("href", data.seo.canonicalUrl);
        document.querySelector('meta[property="og:image"]')?.setAttribute("content", data.seo.openGraphImage);
        document.querySelector('meta[property="og:title"]')?.setAttribute("content", data.seo.metaTitle);
        document.querySelector('meta[property="og:description"]')?.setAttribute("content", data.seo.metaDescription);
        document.querySelector('meta[property="og:url"]')?.setAttribute("content", data.seo.canonicalUrl);
        document.querySelector('meta[name="twitter:title"]')?.setAttribute("content", data.seo.metaTitle);
        document.querySelector('meta[name="twitter:description"]')?.setAttribute("content", data.seo.metaDescription);
        document.querySelector('meta[name="twitter:image"]')?.setAttribute("content", data.seo.openGraphImage);
        ...
        const serviceSchema = {
            ...
            "hasOfferCatalog": {
                "@type": "OfferCatalog",
                "name": data.seo.offerCatalogName || `${data.hero.title} Capabilities`,
                "itemListElement": data.serviceCards.map(card => ({
                    "@type": "Offer",
        ...
        const schemaParentName = data.seo.breadcrumbParentName || "Services";
        const schemaParentUrl = data.seo.breadcrumbParentUrl || "https://gpspl.co.in/#services";
        const breadcrumbSchema = {
            "@context": "https://schema.org",
            "@type": "BreadcrumbList",
            "itemListElement": [
                { "@type": "ListItem", "position": 1, "name": "Home", "item": "https://gpspl.co.in/" },
                { "@type": "ListItem", "position": 2, "name": schemaParentName, "item": schemaParentUrl },
                { "@type": "ListItem", "position": 3, "name": data.hero.title, "item": data.seo.canonicalUrl }
            ]
        };
    ...
    function render(data) {
        ...
        if (partnerTechnologyPages.includes(data.seo.slug || slug)) {
            root.classList.remove("service-page");
            root.classList.add("partner-tech-page");
            root.innerHTML = renderPartnerTechnologyPage(data, parentName, parentHref);
            initServiceReveal();
    ...
    function initServiceInteractions() {
        const index = root.querySelector(".service-page-index");
        const links = index ? Array.from(index.querySelectorAll("a[href^='#']")) : [];
            ...
            const activeObserver = new IntersectionObserver((entries) => {
                ...
                entries.forEach(entry => {
                    ...
                    links.forEach(link => {
                        ...
                        if (isActive && index) {
                            const navScroller = index.querySelector(".container");
                            if (navScroller && navScroller.scrollWidth > navScroller.clientWidth) {
                                const linkLeft = link.offsetLeft - (navScroller.clientWidth - link.offsetWidth) / 2;
                                navScroller.scrollTo({ left: Math.max(0, linkLeft), behavior: "smooth" });
                            }
                        }
                    });
                });
        ...
        const faqs = Array.from(root.querySelectorAll(".service-faq-list details"));
        faqs.forEach(item => {
        ...
        const interactiveCards = root.querySelectorAll(".service-capability-card, .service-application-grid article, .service-partner-card, .service-project-card, .service-feature-grid article");
    ...
    function planningOverview(data) {
        const applications = (data.applications || []).slice(0, 4).join(", ");
        const services = (data.serviceCards || []).slice(0, 4).map(card => card.title).join(", ");
        return `For ${data.hero.title.toLowerCase()}, GPSPL evaluates the business use case, site readiness, product fit, integration dependencies and support expectations before recommending a solution. This helps clients plan ${services || "technology supply and integration"} for ${applications || "enterprise environments"} with clearer budgeting, implementation ownership and support planning.`;
    }
    ...
    function featureCopy(value = "") {
        ...
        if (lower.includes("brand") || lower.includes("oem")) return { title: "Brand & Warranty Support", text: "We help with product selection, warranty guidance and service coordination.", icon: "fa-shield-alt" };
